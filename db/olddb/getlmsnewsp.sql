...
BEGIN DECLARE inc INT DEFAULT 0;
...
*/ SET @fDate= DATE_SUB(cdate, INTERVAL DAYOFMONTH(cdate)-1 DAY) ; 
SET @lastDate= LAST_DAY(cdate); 
...
WHILE @fDate <= @lastDate DO 
	...
	SET @leaveCount=0; 
	SET @title='';
	SELECT COUNT(*) INTO @leaveCount FROM usersleavemanagement WHERE employeeId=empid AND @fdate BETWEEN fromDate AND toDate AND STATUS="Approved";
...
/*usersattendance tbl*/ 
	SET @attCount=0;
/*SET@title='';*/
	SELECT COUNT(*) INTO @attCount FROM usersattendance WHERE employeeId=empid AND @fdate=markedTime;

	if @attCount>0 THEN 
		SET @color='Blue'; 
		SET @title='ApprovalLeave'; 
	END if;
...
WHILE inc <=@proCou DO 
	...
	UPDATE temp_table SET title='Holiday',color=@color WHERE sdate=@stDate ; SET inc=inc+1; 
END WHILE;

/* title emty ku lop update*/
UPDATE temp_table SET title='LOP'
WHERE title='';
SELECT *
FROM temp_table;
DROP TEMPORARY TABLE IF EXISTS temp_table; END//
DELIMITER ;
